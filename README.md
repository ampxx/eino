...
- Context cancellation propagates cleanly through the graph — wrapping `ctx` with a timeout via `context.WithTimeout` is the easiest way to add a global deadline to a pipeline run.
